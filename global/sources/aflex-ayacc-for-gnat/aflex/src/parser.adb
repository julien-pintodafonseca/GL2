...
begin
	...
	if ( sceof(actvsc(i)) ) then
	    ...
	    sceof(actvsc(i)) := true;
	    Ada.Text_IO.put( temp_action_file, "YY_END_OF_BUFFER +" );
	    tstring.put( temp_action_file,  scname(actvsc(i)) );
	    Ada.Text_IO.put_line( temp_action_file, " + 1 " );
	    if (i /= actvp) then
...
use  Parse_Goto, Parse_Shift_Reduce, Ada.Text_IO, misc_defs, tstring;
...
--#line  44
 -- add default rule
			...
			if ( spprdflt ) then
			    Ada.Text_IO.put(temp_action_file,
					"raise AFLEX_SCANNER_JAMMED;");
			else
			    Ada.Text_IO.put( temp_action_file, "ECHO" );
